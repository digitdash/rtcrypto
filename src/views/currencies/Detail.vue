<template>
  <div class="d-header d-flex gap-4 align-items-center mb-4">
    ...
    <div v-if="ticker.price" class="d-ticker ms-auto">
      <div class="display-6 fw-bold">${{ ticker.localePrice }}</div>
      <div v-bind:class="'fw-bold fs-6 lh-sm text-end color-' + ticker.direction"><arrow v-bind:direction="ticker.direction" /> {{ ticker.change }}%</div>
    </div>
  </div>
  <div class="row gy-3">
